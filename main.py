...
def process_proxy(ip, port):
    ...
    if ori and not ori.get("error") and pxy and not pxy.get("error") and ori.get("clientIp") != pxy.get("clientIp"):
        ...
        proxy_country_code = pxy.get("country", "Unknown")
        proxy_asn = pxy.get("asn", "Unknown")
        proxy_latitude = pxy.get("latitude", "Unknown")
        proxy_longitude = pxy.get("longitude", "Unknown")
        ...
        return True, result_message, proxy_country_code, proxy_asn, proxy_country_name, pxy_protocol, org_name, pxy_connection_time, proxy_latitude, proxy_longitude
    else:
        dead_message = f"Cloudflare Proxy Dead: {ip}:{port}"
        print(dead_message)
        return False, dead_message, "Unknown", "Unknown", "Unknown", "Unknown", "Unknown", 0, "Unknown", "Unknown"